 ...
 * modification rather than an unrelated delete + insert. A global first pass (Pass A) heals identical
 * pairs — any string present on both sides (by multiplicity) is unchanged and dropped from all
 * outputs regardless of iteration order, which also repairs rare local sort-order glitches from the
 * attempts path. Pass B then blocks the remainders by first token (genus) and greedily pairs the
 * best match above threshold, so matching is near-linear instead of full quadratic.
 */
