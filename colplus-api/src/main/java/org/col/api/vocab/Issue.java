...
public enum Issue {
    ...
    /**
     * Escaped characters such as html entities have been unescaped during interpretation. The
     * following escaped character formats are recognized: 1) named, decimal or hexadecimal xml &
     * html entities 2) unicode entities U+0026 3) hexadecimal or octal java unicode entites 4) CSS
     * & ECMA Javascript entities
     */
    ESCAPED_CHARACTERS(Group.DATA),

    /**
     * Identifier for a reference could not be resolved. This can be from any entity, e.g. name,
     * distribution, vernaculars, etc
     */
    REFERENCE_ID_INVALID(Group.DATA, Level.ERROR),

    /**
     * A taxon, name or reference with duplicate ids. We remove the subsequent records but flag the
     * first record with this issue.
     */
    ...
    /**
     * The scientific name string could not be parsed at all, but appears to be a parsable name
     * type, i.e. it is not classified as a virus or hybrid formula.
     */
    UNPARSABLE_NAME(Group.NAME),

    /**
     * The beginning of the scientific name string was parsed, but there is additional information
     * in the string that was not understood.
     */
    ...
    /**
     * An parsed, but inconsistent name. E.g. the rank of the name does not match the given name
     * parts or suffices.
     */
    INCONSISTENT_NAME(Group.NAME),

    /**
     * The parsed scientific name string results in a different name then was given as already
     * atomized parts.
     */
    ...
    /**
     * A recombination with a basionym authorship which does not match the authorship of the linked
     * basionym.
     */
    BASIONYM_AUTHOR_MISMATCH(Group.NAME, Level.ERROR),

    /**
     * Record has a original name (basionym) relationship which was derived from name & authorship
     * comparison, but did not exist explicitly in the data. Provisional catalogue specific issue.
     */
    ...
    /**
     * A scientific name has been used to point to another record (synonym->accepted,
     * combination->basionym) which is not unique and refers to several records.
     */
    ...
    /**
     * A bi/trinomial name published earlier than the parent genus was published. This might
     * indicate that the name should rather be a recombination.
     */
    ...
    /**
     * The name (authorship) contains unmatched brackets, i.e. it misses closing brackets usually
     * indicating truncated data.
     */
    ...
    /**
     * An accepted taxon which has a synonym as its parent. This relation is moved to the synonyms
     * accepted name if possible, otherwise just removed.
     */
    SYNONYM_PARENT(Group.TAXON, Level.ERROR),

    /**
     * The given ranks of the names in the classification hierarchy do not follow the hierarchy of
     * ranks.
     */
    CLASSIFICATION_RANK_ORDER_INVALID(Group.TAXON, Level.ERROR),

    /**
     * The denormalized classification could not be applied to the name usage. For example if the id
     * based classification has no ranks.
     */
    CLASSIFICATION_NOT_APPLIED(Group.TAXON),

    /**
     * The (accepted) bi/trinomial name does not match the parent genus/species. For example the
     * species Picea alba with a parent genus Abies is a mismatch.
     */
    PARENT_NAME_MISMATCH(Group.TAXON, Level.ERROR),

    /**
     * The taxonomic status was programmatically derived. E.g. a synonym made a misapplied name or
     * ambigous synonym based on name annotations.
     */
    DERIVED_TAXONOMIC_STATUS(Group.TAXON),

    /**
     * The given taxonomic status appears unlikely. E.g. a misapplied name without any taxonomic
     * remark indicating the source it was misapplied in.
     */
    ...
    /**
     * A vernacular name extension record attached to this name usage is empty or clearly not a
     * name.
     */
    VERNACULAR_NAME_INVALID(Group.VERNACULAR, Level.ERROR),

    /**
     * Transliterated vernacular name missing and generated automatically. Assembled Catalogue
     * specific issue.
     */
   ...
   */
  public enum Group {
      DATA, NAME, TAXON, DISTRIBUTION, VERNACULAR, REFERENCE
  }
