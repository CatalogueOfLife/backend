...
public interface DatasetMapper extends CRUD<Integer, Dataset>, GlobalPageable<Dataset>, DatasetAgentMapper {
  ...
  /**
   * Updates only the identifiers of a dataset.
   */
  void updateIdentifiers(@Param("key") int key, @Param("identifiers") List<Identifier> identifiers);
  ...
  /**
   * Updates only the version DOI of a dataset.
   * @param key
   * @param doi
   */
  void updateVersionDOI(@Param("key") int key, @Param("doi") DOI doi);
   ...
   * @return
   */
  Integer nextRelease(@Param("key") Integer key);
  ...
  /**
   * @return list of unique publisher keys
   */
  List<UUID> listPublisherKeys();
