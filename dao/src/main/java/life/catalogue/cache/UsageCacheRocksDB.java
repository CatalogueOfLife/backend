 ...
 */
public class UsageCacheRocksDB implements UsageCache {
  private static final Logger LOG = LoggerFactory.getLogger(UsageCacheRocksDB.class);
  ...
  /**
   * @param location the parent directory for storing dataset specific "rocks"
   */
  public UsageCacheRocksDB(File location, int kryoMaxCapacity) throws IOException {
  ...
  public SimpleNameCached put(int datasetKey, SimpleNameCached usage) {
    ...
    var store = datasets.computeIfAbsent(datasetKey, (dk) -> {
      ...
      var map = storeMaker(datasetKey, false).createOrOpen();
