...
public abstract class MappingsFactory {
  ...
  /**
   * Whether to map enums to Elasticsearch's integer datatype or to the keyword datatype. Default true. This is more or less separate from
   * how you <i>serialize</i> enums. Obviously when serializing enums to strings, you have no choice but to use the keyword datatype. But
   * when serializing enums to integers, you still have the choice of storing them as strings or as integers. And there can be good reasons
   * to store integers as strings (see Elasticsearch performance tuning guide). Specifying the datatype mapping here saves you from having
   * to decorate each and every enum in the data model with the @MapToType annotation. You can still use the @MapToType to override the
   * global behaviour. Note that in {@link EsModule} we specify that we want enums to be serialized as integers, so we are <i>indeed</i>
   * free to choose between the keyword and integer datatype.
   */
   ...
   * @param ancestors All Java types in the ancestry of the (sub-) document, all the way up to the Java type that the document type mapping
   *        itself was modeled after. Used to detect circular graphs.
   */
  protected abstract void addFields(ComplexField document, Class<?> type, Set<Class<?>> ancestors);

  /**
   * Returns the Java type that should be used to look up the the Elasticsearch datatype in the {@link DataTypeMap}. For arrays and
   * Collection classes we map the element type, not the type itself. Elasticsearch doesn't have an array/list type. Fields are
   * intrinsically multi-valued.
   */
