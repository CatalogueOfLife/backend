package life.catalogue.es2.indexing;
...
public class NameUsageIndexServiceEs implements NameUsageIndexService {
  ...
  public Stats indexDatasets(List<Integer> keys) {
    ...
    LOG.info("Successfully indexed {} datasets. Index: {}. Usages: {}. Bare names: {}. Total: {}.",
      counter, esConfig.index.name, total.usages, total.names, total.total());
    return total;
  }
  ...
  private Stats indexDatasetInternal(int datasetKey, boolean clearIndex) {
    try {
      ...
      LOG.info("Successfully indexed dataset {} into index {}. Usages: {}. Bare names: {}. Total: {}.",
        datasetKey, esConfig.index.name, stats.usages, stats.names, stats.total());
      return stats;
  ...
  public int deleteDataset(int datasetKey) {
    try {
      LoggingUtils.setDatasetMDC(datasetKey, getClass());
      LOG.info("Removing dataset {} from index {}", datasetKey, esConfig.index.name);
      int cnt = EsUtil.deleteDataset(client, esConfig.index.name, datasetKey);
      LOG.info("Deleted all {} documents from dataset {} from index {}", cnt, datasetKey, esConfig.index.name);
      EsUtil.refreshIndex(client, esConfig.index.name);
      return cnt;
  ...
  public Stats indexSector(DSID<Integer> sectorKey) {
    ...
    try (SqlSession session = factory.openSession()) {
      ...
      LOG.info("Successfully indexed sector {}. Index: {}. Usages: {}. Bare names: {}. Total: {}.",
        sectorKey, esConfig.index.name, stats.usages, stats.names, stats.total());
      return stats;
  ...
  public void deleteSector(DSID<Integer> sectorKey) {
    try {
      LoggingUtils.setSectorMDC(sectorKey);
      int cnt = EsUtil.deleteSector(client, esConfig.index.name, sectorKey);
      LOG.info("Deleted all {} documents from sector {} from index {}", cnt, sectorKey, esConfig.index.name);
      EsUtil.refreshIndex(client, esConfig.index.name);
  ...
  public int deleteBareNames(int datasetKey) {
    try {
      ...
      int cnt = EsUtil.deleteBareNames(client, esConfig.index.name, datasetKey);
      LOG.info("Deleted all {} bare name documents from dataset {} from index {}", cnt, datasetKey, esConfig.index.name);
      EsUtil.refreshIndex(client, esConfig.index.name);
      return cnt;
    } finally {
      LoggingUtils.removeDatasetMDC();
  ...
  public void deleteSubtree(DSID<String> root, boolean keepRoot) {
    int cnt = EsUtil.deleteSubtree(client, esConfig.index.name, root, keepRoot);
    LOG.info("Deleted {} documents for entire subtree of root taxon {} from index {}", cnt, root, esConfig.index.name);
    EsUtil.refreshIndex(client, esConfig.index.name);
  }
  ...
  public void delete(DSID<String> usageId) {
    if (usageId != null) {
      ...
      EsUtil.deleteNameUsages(client, esConfig.index.name, usageId.getDatasetKey(), List.of(usageId.getId()));
      EsUtil.refreshIndex(client, esConfig.index.name);
    }
  }
  ...
  public void update(int datasetKey, Collection<String> usageIds) {
    if (!usageIds.isEmpty()) {
      ...
      int deleted = EsUtil.deleteNameUsages(client, esConfig.index.name, datasetKey, usageIds);
  ...
  private void createOrEmptyIndex(int datasetKey) throws IOException {
    if (EsUtil.indexExists(client, esConfig.index.name)) {
      EsUtil.deleteDataset(client, esConfig.index.name, datasetKey);
      EsUtil.refreshIndex(client, esConfig.index.name);
    } else {
      EsUtil.createIndex(client, esConfig.index);
    }
  }
