...
public class TreeDaoTest {
  ...
  TreeDao dao = new TreeDao(SqlSessionFactoryRule.getSqlSessionFactory());
  ...
  public void root() {
    for (boolean placeholder : List.of(false, true)) {
      for (TreeNode.Type type : TreeNode.types()) {
        ResultPage<TreeNode> root = noSectors(dao.root(catKey, catKey, placeholder, true, true, type, PAGE));
        assertEquals(1, root.size());
        assertEquals("1", root.getResult().get(0).getId());
        assertEquals("Animalia", root.getResult().get(0).getName());

        root = noSectors(dao.root(catKey, catKey, placeholder, true, false, type, PAGE));
        assertEquals(1, root.size());
        assertEquals("1", root.getResult().get(0).getId());
        assertEquals("Animalia", root.getResult().get(0).getName());

        root = noSectors(dao.root(TRILOBITA, catKey, placeholder, true, true, type, PAGE));
        assertEquals(1, root.size());
        assertEquals("1", root.getResult().get(0).getId());
  ...
  public void classification() {
    for (boolean placeholder : List.of(false, true)) {
      for (TreeNode.Type type : TreeNode.types()) {
        List<TreeNode> classification = valid(dao.classification(DSID.of(TRILOBITA, "10"), catKey, true, placeholder, true, type));
    ...
    // expect placeholders
    for (TreeNode.Type type : TreeNode.types()) {
      List<TreeNode> classification = valid(dao.classification(DSID.of(TRILOBITA, "61"), catKey, true, true, true, type));
      verifyClassification(classification);
    ...
    for (TreeNode.Type type : TreeNode.types()) {
      List<TreeNode> classification = valid(dao.classification(DSID.of(TRILOBITA, RankID.buildID("2", Rank.FAMILY)), catKey, true, true, true, type));
      verifyClassification(classification);
    ...
    for (TreeNode.Type type : TreeNode.types()) {
      List<TreeNode> classification = valid(dao.classification(DSID.of(catKey, "18"), catKey, true, true, true, type));
  ...
  public void children() {
    ...
    for (TreeNode.Type type : TreeNode.types()) {
      ...
      ResultPage<TreeNode> children = valid(dao.children(key, catKey, true, true, true, type, PAGE));
      ...
      children = valid(dao.children(key, catKey, true, true, true, type, PAGE));
      verifyChildren(key, children);
      assertEquals(2, children.size());
