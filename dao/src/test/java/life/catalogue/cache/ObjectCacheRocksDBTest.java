...
public class ObjectCacheRocksDBTest {
  ...
  public void crud() throws Exception {
    ...
    try (TempFile dir = new TempFile();
         ObjectCache<NameUsageWrapper> cache = new ObjectCacheRocksDB<>(NameUsageWrapper.class, dir.file, pool)
    ) {
      t.setId("a");
  ...
  void test(int size) throws Exception {
    final ApiKryoPool pool = new ApiKryoPool(8);
    try (TempFile dir = new TempFile();
         //ObjectCache<NameUsageWrapper> cache = new ObjectCacheRocksDB<>(NameUsageWrapper.class, dir.file, pool)
         //ObjectCache<NameUsageWrapper> cache = new ObjectCacheMapDB<>(NameUsageWrapper.class, dir.file, pool)
         ObjectCache<NameUsageWrapper> cache = new ObjectCacheMemDB<>(NameUsageWrapper.class, pool)
    ) {
  ...
  public static void main(String[] args) throws Exception {
    ObjectCacheRocksDBTest test = new ObjectCacheRocksDBTest();
    //test.test(10_000);
    //test.test(1_000_000);
    test.test(10_000_000);
  }
}