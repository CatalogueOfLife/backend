...
public class NameUsageIndexServiceTest extends EsReadWriteTestBase {
  ...
  public void indexBulk() throws IOException, EsException {
    try (RestClient client = getEsClient()) {
      ...
      List<NameUsageWrapper<?>> nus = new ArrayList<>();
      BareName bn = new BareName();
      Name name = new Name();
      name.setGenus("Santa");
      name.setSpecificEpithet("clausa");
      bn.setName(name);
      nus.add(new NameUsageWrapper<>(bn));
      nus.add(new NameUsageWrapper<>(TestEntityGenerator.SYN1));
      nus.add(new NameUsageWrapper<>(TestEntityGenerator.SYN2));
      nus.add(new NameUsageWrapper<>(TestEntityGenerator.TAXON1));
      nus.add(new NameUsageWrapper<>(TestEntityGenerator.TAXON2));
      NameUsageIndexService svc = new NameUsageIndexService(client, getEsConfig(), factory(), false);
