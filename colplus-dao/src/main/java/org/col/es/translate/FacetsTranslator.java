 ...
 * by still making it take place within a separate execution context with exactly the same query as the main query. This allows for huge
 * streamlining of the code because the response from Elasticsearch will now always look the same. Besides, it's an unlikely scenario (there
 * will probably alsway be more than one facet). And also, since evrything takes place in a filter context, Elasticsearch will probably
 * cache the filter, thus reducing the performance overhead.
 */
class FacetsTranslator {
  ...
  Map<String, Aggregation> translate() {
    ...
    for (NameSearchParameter facet : copy.getFacets()) {
      ...
      // Temporarily remove the filter corresponding to the facet (if any), oterwise the values retrieved for the facet would collapse to
      // those specified by thee filter.
