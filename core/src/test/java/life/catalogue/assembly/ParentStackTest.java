...
public class ParentStackTest {
  ...
  public void testStack() throws Exception {
    ...
    parents.push(src(1, null), null);
    parents.push(src(2, 1), null);
    var nub = match("nub#3");
    ...
    parents.push(src(3, 2), null);
    assertEquals(3, parents.size());
    assertEquals(nub, parents.lowestParentMatch());
    assertTrue(parents.isDoubtful());

    parents.push(src(4, 1), null); // this removes all but the first key
    assertEquals(2, parents.size());
    assertFalse(parents.isDoubtful());
  ...
  public void testInvalidRankOrder() throws Exception {
    ...
    parents.push(src(Rank.KINGDOM, 1,0), null);
    parents.push(src(Rank.PHYLUM, 2,1), null);
    assertEquals(2, parents.size());
    assertEquals("2", parents.last().usage.getId());
    assertFalse(parents.isDoubtful());

    parents.push(src(Rank.SUPERPHYLUM, 3,2), null);
    assertEquals(3, parents.size());
    assertEquals("3", parents.last().usage.getId());
    assertTrue(parents.isDoubtful());

    parents.push(src(Rank.SUPERPHYLUM, 4,1), null);
    assertEquals(2, parents.size());
    assertEquals("4", parents.last().usage.getId());
    assertFalse(parents.isDoubtful());

    // ambiguous ranks, botany
    parents.push(src(Rank.GENUS, 5,4), null);
    parents.push(src(Rank.SECTION_BOTANY, 6,5), null);
    ...
    // zoological way
    parents.push(src(Rank.ORDER, 8,4), null);
    parents.push(src(Rank.SECTION_ZOOLOGY, 9,8), null);
    parents.push(src(Rank.FAMILY, 10,9), null);
    assertEquals(5, parents.size());
