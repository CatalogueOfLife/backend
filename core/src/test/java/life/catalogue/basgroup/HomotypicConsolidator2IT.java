...
public class HomotypicConsolidator2IT {
  ...
  final TestDataRule dataRule = TestDataRules.homotypigGrouping();
  ...
  public void synSyns() throws IOException {
    try (var hc = HomotypicConsolidator.forTaxa(SqlSessionFactoryRule.getSqlSessionFactory(), dataRule.testData.key,
  ...
  public void consolidateNoPrios() throws Exception {
    printTree();
    try (var hc = HomotypicConsolidator.entireDataset(SqlSessionFactoryRule.getSqlSessionFactory(), dataRule.testData.key, u->1)) {
      hc.consolidate();
