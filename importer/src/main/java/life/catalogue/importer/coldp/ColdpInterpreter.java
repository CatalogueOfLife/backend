...
public class ColdpInterpreter extends InterpreterBase {
  ...
  Optional<NameUsageData> interpretNameUsage(VerbatimRecord v) {
    ...
    return interpretName(v).map(nn -> {
      ...
      UsageData u;
      var statusNote = parse(TaxonomicStatusParser.PARSER, v.get(ColdpTerm.status)).orElse(ACC_NOTE, Issue.TAXONOMIC_STATUS_INVALID, v);
      TaxonomicStatus status = statusNote.val;
      ...
      NameInterpreter.deriveNomStatus(nn.getName(), statusNote.nomStatus, v);
