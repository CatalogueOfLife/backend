...
public class NormalizerACEFIT extends NormalizerITBase {
  ...
  public void acef0() throws Exception {
    normalize(0);
    UsageData t = usageByID("s7");
    // synonym has no accepted thus gets removed!
    assertNull(t);

    NameData n = nameByID("s7");
  ...
  public void acef7Nulls() throws Exception {
    normalize(7);
    UsageData t = usageByID("CIP-S-902");
  ...
  public void acefNameIssues() throws Exception {
    normalize(9);
    UsageData u = usageByID("Lamprostiba_pu!chra");
  ...
  public void acefGenusOnly() throws Exception {
    ...
    t = byName(Rank.GENUS, "Onthophagus");
  ...
  public void ambiguous() throws Exception {
    normalize(19);
    debug();
    UsageData syn = usageByID("S-1025");
  ...
  public void flow() throws Exception {
    ...
    var usages = store.usageIDsByName("Coleoscyta†", null, Rank.GENUS, true);
    assertTrue(usages.isEmpty());
    u = byName(Rank.GENUS, "Coleoscyta");
    assertEquals("Coleoscyta", u.usage.getName().getLabel());

    usages = store.usageIDsByName("Coleoscytidae†", null, Rank.FAMILY, true);
    assertTrue(usages.isEmpty());
    u = byName(Rank.FAMILY, "Coleoscytidae");
    assertEquals("Coleoscytidae", u.usage.getName().getLabel());
  }

  private void assertProParte(UsageData syn, String... acceptedNames) {
  ...
  void assertPlaceholderInParents(String id) {
    UsageData t = usageByID(id);
    for (var u : store.usages().parents(t)) {
      NameData n = store.names().objByID(u.nameID);
