...
import org.col.matching.authorship.Equality;
 ...
 */
public class NameIndex implements AutoCloseable {
   ...
   */
  public NameIndex(DBMaker.Maker dbMaker, AuthorComparator authComp, int datasetKey, @Nullable SqlSessionFactory sqlFactory) {
    ...
    if (names.size() == 0) {
      LOG.info("Loading new names index from postgres");
      // TODO: load index from postgres
    }
  }
  ...
  void addAll(Iterable<Name> names) {
    for (Name n : names) {
      add(n);
    }
  }
